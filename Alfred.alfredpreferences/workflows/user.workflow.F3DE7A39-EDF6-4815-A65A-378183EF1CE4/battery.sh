...
OS_VERSION=$(sw_vers | grep ProductVersion | cut -d"." -f2)

# Default values for MacOS 10.14 and up
CURRENT_CAPACITY=$(cat info.txt | grep AppleRawCurrentCapacity | awk '{printf ("%i", $3)}')
MAX_CAPACITY=$(cat info.txt | grep AppleRawMaxCapacity | awk '{printf ("%i", $3)}')

if [$OS_VERSION -le 13] #Backwards compatibility for MacOS 10.13 and below
then
...
DESIGN_CAPACITY=$(cat info.txt | grep -m1 DesignCapacity | awk '{printf ("%i", $3)}')
